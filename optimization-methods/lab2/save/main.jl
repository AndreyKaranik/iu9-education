...
function coordinate_descent(f, x0; step_size=0.1, tol=1e-6, max_iter=10000)
    x = copy(x0)
    n = length(x0)
    iter_count = 0
    iter_vectors = [copy(x)]
...
function hooke_jeeves(f, x0; Δ=1.0, β=0.5, tol=1e-6, max_iter=10000)
    x = copy(x0)
    n = length(x)
